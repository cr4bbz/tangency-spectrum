...
\begin{remark}[Präzise verbleibende Erneuerungsfrage]
Zerlege jedes Wort der Tiefe $k$ eindeutig als $vL^m$ oder $vR^m$, wobei
der Endlauf maximal ist. Sei $j=|v|$ die Präfixtiefe. Die Konjektur folgt
bereits aus der terminalen Straffheit
\[
 \lim_{J\to\infty}\limsup_{k\to\infty}
 k^s\sum_{\substack{|w|=k\\ j(w)>J}}
 \left(\frac{2}{u_wv_w}\right)^s=0.
\]
Denn für jedes feste Präfix liefert der verbleibende Endlauf genau den
in Proposition~11.12 berechneten Grenzbeitrag. Damit ist die noch offene
obere Schranke als induzierte parabolische Erneuerungsaussage formuliert,
nicht mehr als unbestimmte Abschätzung über alle Wörter.
\end{remark}
