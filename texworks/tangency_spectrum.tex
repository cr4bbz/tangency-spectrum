...
Für $\operatorname{Re}s>1$ und $|z|<1$ gilt
...
Wortreihe absolut konvergent. Die Zerlegung darf daher nach Zahl und
...
Eigenwert $1$. Die exakte Identität
\[
 \mathscr D(s,1)=2^s\frac{\zeta(s)^2}{\zeta(2s)}
 \sim\frac{12}{\pi^2}(s-1)^{-2}
\]
zeigt, dass die vollständige Referenzzeta dort einen Doppelpol hat.
Aus der Resolventendarstellung allein folgt die Doppelpolordnung erst,
wenn die Einfachheit dieser Resonanz und die Nichtverschwindung ihrer
Kopplung an die terminale Beobachtung bewiesen sind. Genau das ist der
nächste, nun präzise formulierte Resonanzschritt.
\end{remark}
