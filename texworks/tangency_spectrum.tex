...
\subsection{Apollonische Tiefenzeta: der nächste Operatorblock}

Für eine normalisierte lokale Apollonische Packung $\mathcal A_\theta$
wählen wir die erste freie Descartes-Geburt als Tiefe $0$. Jeder neu
entstehende Kreis erzeugt drei neue Dreieckslücken. Mit
$\operatorname{dep}(C)$ als Geburtsgeneration definieren wir die
apollonische Tiefenzeta
\[
 \mathscr R_\theta(s,z)
 =
 \sum_{C\in\mathcal A_\theta}
 z^{\operatorname{dep}(C)}\operatorname{rad}(C)^s.
\]
Bei $z=1$ ist dies die lokale Radiuszeta mit kritischem Exponenten
$\delta$.

Die endliche Dreiast-Kodierung ist an den drei Kuspen parabolisch und
daher für eine direkte Spektralanalyse ungeeignet. Der richtige
nächste Schritt ist wiederum Induktion: Ein cuspider Lauf der Länge $m$
wird zu einem Zweig $T_{\theta,m}$ einer abzählbaren, gleichmäßig
kontrahierenden Familie. Diese induzierte Sicht auf den Apollonischen
Gasket liegt auch der rigorosen Dimensionsanalyse von
\href{https://arxiv.org/abs/2406.04922}{Vytnova--Wormell} zugrunde.
Seine ursprüngliche Geburtslänge sei $\tau_{\theta,m}$.

\begin{figure}[htbp]
 \centering
 \input{figures/apollonian_induction.tikz}
 \caption{Apollonische Geburtsdynamik und ihr induzierter
 Kuspmechanismus. Die Dreiast-Verfeinerung erzeugt nach Induktion
 abzählbar viele kontrahierende Rückkehrzweige; die parabolischen
 Potenzen liefern deren quadratischen Schwanz.}
 \label{fig:apollonian-induction}
\end{figure}

\begin{proposition}[Apollonischer Kusp-Schwanz]
In einer Standardnormalisierung des Apollonischen Gaskets sei
\[
 f(z)=\frac{(\sqrt3-1)z+1}{-z+\sqrt3+1},
 \qquad
 R(z)=e^{2\pi i/3}z.
\]
Die induzierten Zweige können als zwei Familien
\[
 T_{2m+1}=f\circ R\circ f^m,
 \qquad
 T_{2m}=f\circ R^2\circ f^m
 \qquad(m\ge1)
\]
geschrieben werden. Auf jedem kompakten Rückkehrgebiet, das den
parabolischen Fixpunkt $1$ nicht enthält, gilt
\[
 \|T_j'\|_\infty\asymp j^{-2},
 \qquad
 \tau_j\asymp j.
\]
Nach den uniformen Möbius-Normierungen der lokalen Wurzellücken gelten
diese Abschätzungen gleichmäßig für
$\theta\in[\sqrt2,q]$.
\end{proposition}

\begin{proof}
Die Matrix von $f$ ist bis auf den Skalar $\sqrt3$ unipotent. Daher
gilt exakt
\[
 f^m(z)
 =
 \frac{(\sqrt3-m)z+m}{-mz+\sqrt3+m},
 \qquad
 (f^m)'(z)
 =
 \frac3{\bigl(\sqrt3+m(1-z)\bigr)^2}.
\]
Auf einem kompakten Rückkehrgebiet ist $|1-z|$ von $0$ getrennt.
Folglich ist $|(f^m)'(z)|\asymp m^{-2}$. Die beiden festen äußeren
Möbius-Abbildungen besitzen auf den relevanten kompakten Bildern
gleichmäßig beschränkte, von $0$ getrennte Ableitungen. Dies beweist
die quadratische Zweigordnung. Die ursprünglichen Wörter enthalten
einen parabolischen Lauf der Länge $m$ und nur endlich viele weitere
Generatoren, also $\tau_j=m+O(1)$. Die in
Proposition~8.2 bewiesenen Möbius-Normierungen bewegen die relevanten
Kompakta nur in einer kompakten Familie und übertragen die
Abschätzungen uniform auf den angegebenen Parameterbereich.
\end{proof}

Der gewichtete induzierte Operator besitzt damit die Form
\[
 (\mathcal A_\theta(s,z)f)(x)
 =
 \sum_{m\ge1}
 z^{\tau_{\theta,m}}
 |T_{\theta,m}'(x)|^s f(T_{\theta,m}(x)).
\]
Sein führender Eigenwert werde mit $\Lambda_\theta(s,z)$ bezeichnet.

\begin{proposition}[Konditionale lineare Crossoverform]
Angenommen, der führende
Eigenwert bei $(\delta,1)$ ist einfach. Dann existieren positive
Konstanten
\[
 \chi_\theta=-\partial_s\Lambda_\theta(\delta,1),
 \qquad
 \mu_\theta=\partial_z\Lambda_\theta(\delta,1)
\]
und es gilt
\[
 1-\Lambda_\theta(s,z)
 ...
 \chi_\theta(s-\delta)
 +\mu_\theta(1-z)
 +o\!\left(|s-\delta|+|1-z|\right).
\]
Für eine passende terminale Beobachtung folgt daraus eine lineare
kritische Skala
\[
 (1-z)\,
 \mathscr R_\theta\bigl(\delta+\alpha(1-z),z\bigr)
 \longrightarrow
 \frac{A_\theta}{\mu_\theta+\chi_\theta\alpha},
 \qquad \alpha\ge0,
\]
wobei $A_\theta>0$ die Projektion der Beobachtung auf den führenden
Eigenraum ist.
\end{proposition}

\begin{proof}
Der Kusp-Schwanz liefert bei $s=\delta>1$
\[
 \sum_{m\ge1}\tau_{\theta,m}
 \|T_{\theta,m}'\|_\infty^\delta
 \ll
 \sum_{m\ge1}m^{1-2\delta}<\infty.
\]
Der Operator ist daher am Punkt $z=1$ einmal nach $z$
differenzierbar. Analytische Störungstheorie am einfachen Eigenwert
liefert die lineare Entwicklung. Die Resolventendarstellung mit
terminaler Beobachtung ergibt anschließend den skalierten Grenzwert.
\end{proof}

\begin{remark}[Warum dies nicht die Stern--Brocot-Skala ist]
Bei den Randlücken liegt der kritische Exponent bei $s=1$; der
entsprechende erste Rückkehrmoment ist grenzwertig divergent und
erzeugt $(1-z)\log(1/(1-z))$. Für die Apollonische Radiuszeta gilt
$\delta>1$, so dass derselbe parabolische Schwanz einen endlichen
ersten Moment hätte. Die lineare Form wäre daher ein echter
Klassenunterschied zwischen eindimensionaler Tangenzlänge und
zweidimensionaler Kreisradiusdynamik.
\end{remark}
...
\subsubsection{Universelles normiertes Crossoverprofil}

Ordne das Wurzelpaar einer unteren Lücke als $(A_n,A_nx_n)$ mit
\[
 x_n=\theta_n^{-1}\in
 K:=\left[q^{-1},2^{-1/2}\right].
\]
Die natürliche dimensionslose lokale Tiefenzeta ist
\[
 \widehat{\mathscr D}_n(s,z)
 =
 \sum_{k\ge0}z^k
 \sum_{|w|=k}
 \left(\frac{\ell_{n,w}}{\ell_{n,\varnothing}}\right)^s.
\]
Ihre kritische Normierung ist durch die Tiefenerhaltung bereits
festgelegt:
\[
 \widehat{\mathscr D}_n(1,z)=\frac1{1-z}.
\]

\begin{proposition}[Universelles Wurzellückenprofil]
Sei $\alpha\ge0$ fest, $t\downarrow0$ und
\[
 z_t=1-t,
 \qquad
 s_t=1+\alpha t\log\frac1t.
\]
Dann gilt gleichmäßig in $n$
\[
 t\,\widehat{\mathscr D}_n(s_t,z_t)
 \longrightarrow
 \frac{1}{1+\frac{\pi^2}{6}\alpha}.
\]
Das kritische Crossoverprofil ist nach der Wurzelnormierung also
unabhängig von der lokalen Form $\theta_n$.
\end{proposition}

\begin{proof}
Für ein beliebiges Startpaar $(A,Ax)$ mit $x\in K$ liefert die
Quotientendynamik die exakte Transferdarstellung
\[
 \widehat{\mathscr D}_{A,x}(s,z)
 =
 x^s\sum_{k\ge0}z^k
 (\mathcal L_s^k\psi_s)(x),
 \qquad
 \psi_s(y)=y^{-s}.
\]
Bei $s=1$ ist $\psi_1(y)=y^{-1}$ ein fester Vektor von
$\mathcal L_1$. Dies erklärt unmittelbar die Identität
$\widehat{\mathscr D}_{A,x}(1,z)=(1-z)^{-1}$.

Induziert man die Bahn von $x\in K$ auf $Y$, so bilden die
Eingangs- und terminalen Operatoren eine in $x$ kompakte Familie. Die
Rang-eins-Schwanzabschätzung und ihr um eine Potenz besserer Rest gelten
dort gleichmäßig in $x$. Ihre Projektion auf den einfachen
Gauß-Eigenraum ist proportional zu $x^{-1}$; der Faktor $x^s$ in der
Normierung hebt ihn bei $s=1$ auf. Die gesamte Beweisführung der
kritischen Übergangsskala gilt daher uniform in $x\in K$, nun mit
Anfangswert $1$ statt $2$. Da jedes $x_n$ in $K$ liegt, folgt die
Behauptung.
\end{proof}

\begin{figure}[htbp]
 \centering
 \input{figures/critical_crossover.tikz}
 \caption{Universelles kritisches Crossoverprofil der nach ihrer
 Wurzellänge normierten lokalen Tiefenzeta. Die skalierte Tiefe
 $t=1-z$ und die Spektralverschiebung
 $s-1=\alpha t\log(1/t)$ erzeugen das Profil
 $\Phi(\alpha)=2/(1+\pi^2\alpha/6)$ im Referenzfall und die Hälfte
 davon in jeder normierten Wurzellücke.}
 \label{fig:critical-crossover}
\end{figure}
...
{Jenkinson--González--Urba\'nski}; die Konjugationsformel überträgt sie
...
\subsection{Zweiparametrige Normalform am kritischen Punkt}

Nun wird die Tiefe wieder freigegeben. Für reelles $s$ nahe $1$ und
$0<z\le1$ bezeichne $\lambda(s,z)$ den einfachen Eigenwert von
$\mathcal G(s,z)$, der aus $\lambda(1,1)=1$ fortgesetzt wird. Die
Einfachheit am Ausgangspunkt und die Operatornorm-Stetigkeit bis zum
Rand $z=1$ sichern diese lokale Wahl.

\begin{proposition}[Lokale Resonanzform]
Für $s\to1$ und $z\uparrow1$ gilt mit $t=1-z$
\begin{equation}\label{eq:two-parameter-normal-form}
 1-\lambda(s,z)
 =
 \frac{\pi^2}{6\log2}(s-1)
 +
 \frac{t\log(1/t)}{\log2}
 +
 o\!\left(|s-1|+t\log(1/t)\right).
\end{equation}
\end{proposition}

\begin{proof}
Sei wieder $h(x)=x^{-1}$ und
$\nu(f)=\int_{1/2}^1f(x)\,dx$. Für die einzelnen Rückkehrblöcke setze
\[
 a_m=\nu(R_m(1)h)
 =
 \int_{1/2}^1
 \frac{dx}{(1+mx)(1+(m+1)x)}.
\]
Dann gilt
\[
 a_m=(m+1)^{-2}+O\!\left((m+1)^{-3}\right).
\]
Folglich ist
\[
 \begin{aligned}
 \nu\bigl((\mathcal G(1,z)-\mathcal G(1,1))h\bigr)
 &=
 \sum_{m\ge0}(z^{m+1}-1)a_m\\
 &=
 \operatorname{Li}_2(z)-\zeta(2)+O(1-z)\\
 &=
 (1-z)\log(1-z)+O(1-z).
 \end{aligned}
\]
Die Störungsformel für einen einfachen Eigenwert und
$\nu(h)=\log2$ liefern damit
\[
 \lambda(1,z)-1
 =
 \frac{(1-z)\log(1-z)}{\log2}+O(1-z).
\]
Die $s$-Ableitung ist die bereits berechnete Zahl
\[
 \partial_s\lambda(1,1)=-\frac{\pi^2}{6\log2}.
\]
Für die gemischten Reste verwenden wir die Rang-eins-Abschätzung der
Rückkehrblöcke. Sie reduziert deren Kontrolle auf
\[
 \sum_{n\ge1}(1-z^n)n^{-2}\log n
 =
 O\!\left(t\log^2(1/t)\right)
 \qquad(t\downarrow0),
\]
während der operatorielle Rest wegen $O(n^{-3})$ nur $O(t)$ beiträgt.
Zusammen mit dem quadratischen Rest der Störungstheorie ist dies
$o(|s-1|+t\log(1/t))$. Dies ergibt
\eqref{eq:two-parameter-normal-form}.
\end{proof}

\begin{proposition}[Kritische Erhaltung auf jeder Tiefe]
Für jede Tiefe $k\ge0$ gilt exakt
\[
 \mathcal Z_k^{\mathrm{ref}}(1)=2.
\]
Daher ist
\[
 \mathscr D(1,z)=\frac2{1-z}
 \qquad(|z|<1).
\]
\end{proposition}

\begin{proof}
Ein Knoten mit Paar $(u,v)$ trägt bei $s=1$ das Gewicht $2/(uv)$.
Die beiden Kinder $(u,u+v)$ und $(u+v,v)$ besitzen zusammen das Gewicht
\[
 \frac2{u(u+v)}+\frac2{(u+v)v}
 =
 \frac2{uv}.
\]
Die Gesamtsumme bleibt daher unter jeder Verfeinerung erhalten und
hat an der Wurzel den Wert $2$. Die zweite Aussage ist ihre
erzeugende Funktion.
\end{proof}

\begin{remark}[Interpretation der Normalform]
Die zweite Größe in \eqref{eq:two-parameter-normal-form},
\[
 (1-z)\log\frac1{1-z},
\]
ist die parabolische Tiefenskala. Sie konkurriert direkt mit der
Spektralverschiebung $s-1$. Die terminale Beobachtung trägt zugleich
den Faktor $\operatorname{Li}_s(z)$. Im Grenzfall $s=1$ heben sich
dessen logarithmisches Wachstum und die parabolische Skala im Nenner
genau so auf, dass die exakte kritische Antwort
\[
 \mathscr D(1,z)=\frac2{1-z}
\]
entsteht. Im Grenzfall $z=1$ liefert
$\operatorname{Li}_s(1)=\zeta(s)$ zusammen mit der ersten Zeile von
\eqref{eq:two-parameter-normal-form} den Doppelpol in $s$.
\end{remark}


\begin{theorem}[Kritische Übergangsskala]
Sei $\alpha\ge0$ fest, $t\downarrow0$ und
\[
 z_t=1-t,
 \qquad
...
\[
 t\,\mathscr D(s_t,z_t)
 \longrightarrow
 \frac{2}{1+\frac{\pi^2}{6}\alpha}.
\]
\end{theorem}

\begin{proof}
Setze $L=\log(1/t)$. Aus \eqref{eq:two-parameter-normal-form} folgt
\[
 1-\lambda(s_t,z_t)
 =
 \frac{tL}{\log2}
 \left(1+\frac{\pi^2}{6}\alpha+o(1)\right).
\]
Ferner gilt die uniforme Polylogarithmus-Asymptotik
\[
 \operatorname{Li}_{s_t}(z_t)\sim L.
\]
Sie folgt etwa durch Vergleich der Reihe mit dem Integral von
$e^{-tx}x^{-1-\alpha tL}$.

Die Spektralzerlegung der Resolvente am einfachen Eigenwert liefert
\[
 (I-\mathcal G(s_t,z_t))^{-1}
 =
 \frac{P+o(1)}{1-\lambda(s_t,z_t)}+O(1).
\]
Nach der terminalen Regularisierung und
$\nu(x^{-2})=1$ gilt daher
\[
 (P\eta(s_t,z_t;\,\cdot))(1)
 =
 \frac{L}{\log2}(1+o(1)).
\]
Eingesetzt in \eqref{eq:induced-depth-resolvent} ergibt dies
\[
 \mathscr D(s_t,z_t)
 =
 \frac{2}{t}
 \frac{1+o(1)}{1+\frac{\pi^2}{6}\alpha},
\]
wie behauptet.
\end{proof}

\begin{remark}[Universelle lokale Profilfunktion]
Mit der skalierten Koordinate
\[
 \alpha
 =
 \frac{s-1}{(1-z)\log(1/(1-z))}
\]
lautet das lokale Profil
\[
 \Phi(\alpha)=\frac{2}{1+\frac{\pi^2}{6}\alpha}.
\]
Bei $\alpha=0$ reproduziert es die exakte kritische Identität
$\mathscr D(1,z)=2/(1-z)$. Die Steigung der Profilfunktion wird allein
durch die Gauß-Entropiekonstante $\pi^2/(6\log2)$ festgelegt.
\end{remark}
